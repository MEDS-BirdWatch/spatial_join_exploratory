...
bbox <- st_bbox(c(xmin = -125, xmax = -113, ymin = 37, ymax = 42), crs = 4326)
norcal <- st_crop(ca, bbox)
...
veg_norcal <- crop(veg_type,norcal)
```
...
gap_norcal <- st_intersection(gap_clean, norcal)
...
norcal_point_area <- st_intersection(point_area_geo, norcal)
```
